...
# Bản FULL đã tích hợp MULTI-KEY ROTATION + Fallback + Cache
# ==========================================================
...
if not API_KEYS or len(API_KEYS) == 0:
    st.error("⚠️ Bạn chưa cấu hình GEMINI_API_KEYS trong secrets.toml")
    st.stop()
...
def rotate_key():
    """Tự động chuyển sang API key kế tiếp."""
...
def call_gemini_text(model, user_prompt):
    ...
    for m in models_to_try:
        url = (
            ...
            f"{m}:generateContent?key={get_api_key()}"
        )
        ...
        try:
            ...
            if res.status_code == 429:
                rotate_key()
                last_error = f"Quota exceeded → Đổi API key mới."
                continue
    ...
    return None, f"❌ Fallback thất bại. {last_error}"


# ======================
# API — IMAGE with KEY ROTATION
# ======================
def call_gemini_image(model, prompt):
    ...
    for m in models_to_try:
        ...
        try:
            ...
            if res.status_code == 429:
                rotate_key()
                last_error = "Quota exceeded ảnh → đổi API key."
                continue
    ...
    return None, f"❌ Lỗi tạo ảnh (fallback thất bại): {last_error}"
...
if not st.session_state.user_name or not st.session_state.user_class:
    ...
    name_input = col1.text_input("Họ và tên")
    class_input = col2.text_input("Lớp")
...
if st.session_state.pending_action:
    ...
    if st.session_state.pending_action == "text":
        ...
        st.session_state.chat_history.append({"role": "user", "text": f"[Tạo ảnh] {q}"})
...
c1, c2 = st.columns(2)
c1.button("Gửi câu hỏi", on_click=set_pending_action, args=("text",))
c2.button("Tạo ảnh minh họa", on_click=set_pending_action, args=("image",))
